...
import { FileText, Clock, UserPlus } from "lucide-react"; // Added UserPlus, assuming FileText and Clock are also from lucide-react

// Wrapper to style the dialog trigger like the existing buttons
function AddEmployeeDialogTrigger() {
    return (
        <div className="w-full">
            <AddEmployeeDialog trigger={
                ...
                >
                    <div className="h-10 w-10 rounded-lg flex items-center justify-center mr-4 text-blue-600 bg-blue-50 dark:bg-blue-900/20 group-hover:scale-105 transition-transform">
                        <UserPlus className="h-5 w-5" />
                    </div>
                    <div className="text-left">
                        <div className="font-semibold text-sm">New Employee</div>
                        <div className="text-xs text-muted-foreground font-normal">Add to workforce</div>
                    </div>
                </Button>
            } />
        </div>
...
function RunPayrollDialogTrigger() {
    return (
        <RunPayrollDialog trigger={
            <Button size="sm" variant="secondary" className="w-full h-8 text-xs font-semibold bg-white/10 hover:bg-white/20 text-white border-0">
                Process Now
            </Button>
        } />
    )
